...
public class TreeBitmap {
	...
	public String longestPrefixOf(String destinationAddress) {
		numOfNodesVisited = 0;
		LMP = Search (root, destinationAddress, 0);
		if (LMP == null) {
			return null;
		}
		return  LMP.nextHop;
	...
	private Node Search(Node x, String destinationAddress, int b) {
		...
		if( (b + STRIDE) > destinationAddress.length() ) 
			return LMP;
		numOfNodesVisited ++;
		return  Search(x.pointer[LPMUtil.decimalToBinary(destinationAddress.substring(b, b+STRIDE))], destinationAddress, b+STRIDE);	
	...
	public static void main(String[] args) {
		...
		TreeBitmap treeBitmap = new TreeBitmap();
		treeBitmap.insert("", "0.0.0.0");
		treeBitmap.insert("100011100001010110001","1.1.1.1");
		treeBitmap.insert("1001100001111101101110","2.2.2.2");
		System.out.println("============ Test Results =============");
		System.out.println("10001110000101011000100111000 ==>>>> "+ treeBitmap.longestPrefixOf("10001110000101011000100111000"));
		System.out.println("10011000011111011011101001011011 ==>> "+treeBitmap.longestPrefixOf("10011000011111011011101001011011"));
		System.out.println("00011000011111011011101001011011 ==>> "+treeBitmap.longestPrefixOf("00011000011111011011101001011011"));
